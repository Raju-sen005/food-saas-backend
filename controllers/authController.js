...
exports.registerTenant = async (req, res) => {
  ...
  try {
    ...
    if (slugExists) return res.status(400).json({ success: false, message: 'Slug/Custom URL already taken' });
...
exports.login = async (req, res) => {
  ...
  try {
    ...
    res.status(200).json({
      success: true,
      data: { id: user._id, name: user.name, email: user.email, role: user.role, restaurantId: user.restaurantId }
    });
  } catch (error) {
    res.status(500).json({ success: false, message: error.message });
  }
